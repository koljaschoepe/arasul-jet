  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (quote.company_phone) {
        contactParts.push(`Tel: ${quote.company_phone}`);
      }
      if (quote.company_email) {
        contactParts.push(quote.company_email);
      }
      if (quote.company_website) {
        contactParts.push(quote.company_website);
      }
      ...
      doc
        ...
        .text(`Gültig bis: ${formatDate(quote.valid_until)}`, 350, 107, {
          align: 'right',
          width: 195,
        });
      ...
      // === CUSTOMER ADDRESS ===
      doc.font(FONTS.regular).fontSize(9).fillColor(COLORS.muted).text('Empfänger:', 50, y);
      ...
      if (quote.customer_name) {
        doc
          .font(quote.customer_company ? FONTS.regular : FONTS.bold)
          .text(quote.customer_name, 50, y);
        y += 14;
      }

      if (quote.customer_address) {
        doc.font(FONTS.regular).fontSize(10).text(quote.customer_address, 50, y);
      ...
      doc.rect(50, tableTop, pageWidth, 25).fill(primaryColor);
      ...
      for (const pos of positions) {
        const rowHeight =
          25 +
          (pos.description
            ? Math.min(doc.heightOfString(pos.description, { width: colWidths.name }), 30)
            : 0);
        ...
        if (pos.position_number % 2 === 0) {
          doc.rect(50, y - 5, pageWidth, rowHeight).fill(COLORS.background);
        }
        ...
        doc.font(FONTS.bold).text(prefix + pos.name, x, y, { width: colWidths.name });
        ...
        x += colWidths.name;
        doc.text(pos.quantity.toString().replace('.', ','), x, y, {
          width: colWidths.qty,
          align: 'right',
        });
        ...
        x += colWidths.unit;
        doc.text(formatCurrency(pos.unit_price, quote.currency, quote.currency_symbol), x, y, {
        ...
        doc.text(formatCurrency(pos.total_price, quote.currency, quote.currency_symbol), x, y, {
      ...
      doc.text(
        formatCurrency(quote.subtotal, quote.currency, quote.currency_symbol),
        totalsX + 95,
        y,
      ...
      if (parseFloat(quote.discount_amount) > 0) {
        ...
        doc.text(
          `-${formatCurrency(quote.discount_amount, quote.currency, quote.currency_symbol)}`,
          totalsX + 95,
          y,
          { width: 100, align: 'right' }
        );
      ...
      doc.text(
        formatCurrency(quote.tax_amount, quote.currency, quote.currency_symbol),
        totalsX + 95,
        y,
        { width: 100, align: 'right' }
      );
      ...
      doc.text(
        formatCurrency(quote.total, quote.currency, quote.currency_symbol),
        totalsX + 95,
        y,
        { width: 100, align: 'right' }
      );
      ...
      // === NOTES ===
      if (quote.notes) {
        doc.font(FONTS.bold).fontSize(10).fillColor(COLORS.text).text('Anmerkungen:', 50, y);
        y += 15;

        doc.font(FONTS.regular).fontSize(9).text(quote.notes, 50, y, { width: pageWidth });
        y += doc.heightOfString(quote.notes, { width: pageWidth }) + 20;
      }

      // === PAYMENT TERMS ===
      if (quote.pdf_payment_terms) {
        doc.font(FONTS.bold).fontSize(10).text('Zahlungsbedingungen:', 50, y);
        y += 15;
      ...
      if (quote.company_name) {
        footerParts.push(quote.company_name);
      }
      if (quote.company_tax_id) {
        footerParts.push(`USt-IdNr.: ${quote.company_tax_id}`);
      }
