...
const DATA_FUNCTIONS = {
  ...
  async query_table(tableSlug, filters = {}, limit = 20) {
    ...
    try {
      ...
      const tableCheck = await dataDb.query('SELECT id FROM dt_tables WHERE slug = $1', [
        tableSlug,
      ]);
...
async function executeDataFunction(functionName, args = {}) {
  ...
  try {
    ...
    logger.info(`[LLMDataAccess] Executed ${functionName}`, {
      args,
...
async function getAllTableSchemas() {
  try {
    const tablesResult = await dataDb.query('SELECT slug FROM dt_tables ORDER BY name');
...
function buildSQLPrompt(query, schema) {
  const fieldsList = schema.fields
    .map(f => `  - ${f.column} (${f.name}): ${f.type}${f.required ? ', Pflichtfeld' : ''}`)
    .join('\n');

  const systemFieldsList = schema.systemFields
    .map(f => `  - ${f.column} (${f.name}): ${f.type}`)
    .join('\n');
...
async function generateAndExecuteSQL(query, tableSlug = null) {
  ...
  logger.info(
    `[LLMDataAccess] SQL Generation request: "${query}" for table: ${tableSlug || 'auto'}`
  );
