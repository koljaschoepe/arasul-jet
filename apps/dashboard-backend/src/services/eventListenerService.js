...
class EventListenerService {
  constructor() {
    ...
    this.maxReconnectDelay = 60000; // 60s max
  ...
  async startDockerListener() {
    ...
    try {
      ...
      // WS-003: Reset backoff on successful connection
      this.reconnectAttempts = 0;
  ...
  /**
   * WS-003: Schedule Docker listener reconnect with exponential backoff
   */
  scheduleReconnect() {
    this.reconnectAttempts++;
    const delay = Math.min(5000 * Math.pow(2, this.reconnectAttempts - 1), this.maxReconnectDelay);
    logger.info(`Scheduling Docker reconnect in ${delay}ms (attempt ${this.reconnectAttempts})`);
    setTimeout(() => {
      if (!this.isListening) {
        this.startDockerListener().catch(err => {
          logger.error(`Docker reconnect failed: ${err.message}`);
        });
      }
    }, delay);
  }
  ...
  async handleDockerEvent(event) {
    try {
      ...
      const statusChange = await this.updateServiceStatusCache(containerName, action, healthStatus);
  ...
  async updateServiceStatusCache(containerName, action, healthStatus) {
    try {
      ...
      const result = await db.query(
        ...
        [containerName, containerName, status, health, JSON.stringify({ last_action: action })]
      );
  ...
  shouldNotifyDockerEvent(action, statusChange, containerName) {
    ...
    if (
      ...
      (statusChange?.status_changed || statusChange?.health_changed)
    ) {
      return true;
    }
  ...
  async recordSystemBoot() {
    try {
      ...
      // Record in database
      const result = await db.query(`SELECT record_system_boot($1, $2)`, [
        JSON.stringify(servicesStatus),
        null,
      ]);
      ...
      await telegramService.queueNotification({
        ...
        message:
          'Das Arasul Platform System wurde neu gestartet. Alle Services werden initialisiert.',
        metadata: {
          boot_id: bootId,
