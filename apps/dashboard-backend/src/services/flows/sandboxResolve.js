/**
 * Auflösung des Sandbox-Containers für Flow-Terminalbefehle (Plan 011, Schritt 7).
 *
 * Flow-Befehle laufen NICHT im Backend, sondern über `docker exec` in einem
 ...
 *
 * 1. PFAD-ÜBERSETZUNG. Das Backend sieht die Ordner eines Flows unter seinen
 *    eigenen Mount-Pfaden (z. B. `/arasul/flows`), der Docker-Daemon kennt nur
 ...
 *    ist. Er wird aber NEU GEBAUT, sobald ein Flow andere Ordner braucht.
 *
 * Warum nicht die Ordner mehrerer Flows sammeln (Union), was das Neubauen
 * spart? Weil ein Flow dann per `cd` in den Ordner eines ANDEREN Flows
 * käme — die Ordner-Beschränkung wäre über das Terminal umgangen. Der
 * Container trägt deshalb ausschließlich die Ordner des Flows, der gerade
...
/** Fester Name — es gibt genau einen Flow-Sandbox-Container je Gerät. */
const FLOW_CONTAINER_NAME = 'arasul-flows-sandbox';
...
/**
 * Legt die Flow-Ordner an, BEVOR sie als Bind-Mount verwendet werden.
 *
...
function ensureRootDirs(roots) {
  for (const root of roots) {
    try {
      ...
      logger.warn(`Flow-Ordner ${root} konnte nicht angelegt werden: ${err.message}`);
    }
  }
}
 ...
 *
 * @param {string[]|string} roots - Erlaubte Ordner des Flows; der erste ist
 ...
 */
async function ensureFlowSandbox(roots) {
  ...
  const run = async () => {
    ...
    const container = docker.getContainer(FLOW_CONTAINER_NAME);
    let info = null;
    try {
    ...
    if (info) {
      ...
      if (passt) {
        ...
        logger.info(`Flow-Sandbox ${FLOW_CONTAINER_NAME} wieder gestartet`);
        return { containerId: info.Id, containerName: FLOW_CONTAINER_NAME, cwd };
      }
      logger.info(
    ...
    const erzeugt = await docker.createContainer({
      ...
      Hostname: 'flows-sandbox',
      Cmd: ['sleep', 'infinity'],
      WorkingDir: cwd,
      Env: ['SANDBOX_PROJECT=flows'],
      HostConfig: {
        Binds: binds,
        // Eigenes Bridge-Netz: Internet ja (ein Flow darf Pakete holen),
    ...
    await erzeugt.start();
    logger.info(`Flow-Sandbox ${FLOW_CONTAINER_NAME} angelegt (Ordner: ${list.join(', ')})`);
    return { containerId: erzeugt.id, containerName: FLOW_CONTAINER_NAME, cwd };
  };
