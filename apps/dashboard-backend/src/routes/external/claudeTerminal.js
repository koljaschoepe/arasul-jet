...
async function getOrCreateSession(userId) {
  try {
    ...
    if (existingResult.rows.length > 0) {
      ...
      await db.query(`UPDATE claude_terminal_sessions SET last_activity_at = NOW() WHERE id = $1`, [
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Save query to history
    const queryId = session
      ? await saveQueryToHistory(session.id, userId, {
          query,
          context: contextData,
          ...
          for (const line of lines) {
            try {
              ...
              if (parsed.done) {
                ...
                logger.info(
                  `Claude Terminal query completed: ${queryId}, tokens: ${tokenCount}, time: ${responseTime}ms`
                );
              }
            } catch (parseError) {
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    await db.query(`DELETE FROM claude_terminal_queries WHERE user_id = $1`, [userId]);
