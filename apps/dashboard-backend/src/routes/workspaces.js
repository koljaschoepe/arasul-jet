...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existingSlug = await db.query('SELECT id FROM claude_workspaces WHERE slug = $1', [slug]);
    ...
    if (!allowedPrefixes.some(prefix => normalizedPath.startsWith(prefix))) {
      throw new ValidationError(
        `Host-Pfad muss mit einem der folgenden Präfixe beginnen: ${allowedPrefixes.join(', ')}`
      );
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    if (workspace.is_default) {
      throw new ForbiddenError(
        'Der Standard-Workspace kann nicht gelöscht werden. Bitte zuerst einen anderen Standard setzen.'
      );
    }

    // Soft delete
    await db.query('UPDATE claude_workspaces SET is_active = FALSE WHERE id = $1', [id]);
