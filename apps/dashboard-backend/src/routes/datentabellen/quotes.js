...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (customer) {
      conditions.push(
        `(customer_email ILIKE $${paramIndex} OR customer_name ILIKE $${paramIndex} OR customer_company ILIKE $${paramIndex})`
      );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    let template;
    if (template_id) {
      const templateResult = await dataDb.query('SELECT * FROM dt_quote_templates WHERE id = $1', [
        template_id,
      ]);
    ...
    const taxRate = template?.tax_rate || 19.0;
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existingResult = await dataDb.query('SELECT * FROM dt_quotes WHERE id = $1', [quoteId]);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (is_default) {
      await dataDb.query(
        'UPDATE dt_quote_templates SET is_default = FALSE WHERE is_default = TRUE'
      );
    }
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Check exists
    const existingResult = await dataDb.query('SELECT id FROM dt_quote_templates WHERE id = $1', [
      templateId,
    ]);
    ...
    if (req.body.is_default) {
      await dataDb.query(
        'UPDATE dt_quote_templates SET is_default = FALSE WHERE is_default = TRUE AND id != $1',
        [templateId]
      );
    }
