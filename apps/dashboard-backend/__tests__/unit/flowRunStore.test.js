...
describe('cancelRun', () => {
  it('bricht Lauf UND offene Schritte ab, nur für den eigenen Nutzer', async () => {
    ...
    expect(schrittSql.sql).toMatch(/UPDATE flow_run_steps/);
...
describe('getRun', () => {
  ...
  it('lädt Rohdaten nur auf ausdrücklichen Wunsch (includeRaw)', async () => {
    ...
    expect(db.calls[1].sql).toMatch(/SELECT \* FROM flow_run_steps/);
  });
