...
const sandboxResolve = require('../../src/services/flows/sandboxResolve');
...
describe('TerminalTool — was im Container ankommt', () => {
  ...
  it('führt den Befehl im Arbeitsverzeichnis des Flows aus', async () => {
...
describe('sandboxResolve — Pfadübersetzung', () => {
  const mounts = [
    { Source: '/home/arasul/arasul/arasul-jet/data/flows', Destination: '/arasul/flows' },
  ...
  it('übersetzt einen Backend-Pfad in den Host-Pfad', async () => {
    await expect(sandboxResolve.toHostPath('/arasul/flows/bericht')).resolves.toBe(
      '/home/arasul/arasul/arasul-jet/data/flows/bericht'
    );
  });

  it('nimmt den SPEZIELLSTEN Mount, nicht den erstbesten', async () => {
    // /arasul/flows liegt innerhalb von /arasul — der längere Treffer gewinnt,
    // sonst zeigt die Übersetzung auf den falschen Host-Ordner.
    await expect(sandboxResolve.toHostPath('/arasul/flows')).resolves.toBe(
      '/home/arasul/arasul/arasul-jet/data/flows'
    );
  });
...
describe('sandboxResolve — Container-Lebenszyklus', () => {
  ...
  it('legt die Flow-Ordner an, BEVOR Docker sie als root anlegen würde', async () => {
    ...
    // `dateien_schreiben` scheiterte mit EACCES. Deshalb muss ensureFlowSandbox
    ...
    await sandboxResolve.ensureFlowSandbox(roots);
  ...
  it('ein fehlgeschlagenes Anlegen bricht den Lauf nicht ab', async () => {
    ...
    const res = await sandboxResolve.ensureFlowSandbox(roots);
  ...
  it('baut neu auf, wenn der Container ZUSÄTZLICHE Ordner hat', async () => {
    ...
    // eines anderen Flows hängen, darf NICHT weiterverwendet werden — sonst
    // käme dieser Flow per `cd` an fremde Dateien.
    verdrahte(
      jest.fn().mockResolvedValue({
        Id: 'sandbox-alt',
        State: { Running: true },
  ...
  it('nennt das fehlende Sandbox-Image als Ursache, statt abzustürzen', async () => {
    ...
    await expect(sandboxResolve.ensureFlowSandbox(roots)).rejects.toThrow(
      /Sandbox-Image .* fehlt/i
    );
  });

  it('weist einen Flow-Ordner ab, der im Backend gar nicht gemountet ist', async () => {
    verdrahte(jest.fn().mockResolvedValue({}));
    await expect(sandboxResolve.ensureFlowSandbox(['/irgendwo/anders'])).rejects.toThrow(
      /nicht als Mount/i
    );
  });
});
