 ...
 * Auswahlliste aus dem Flow, Wissensbasis aus der API; Label ≠ Wert.
 */
...
describe('ArgumentPicker · Auswahlliste', () => {
  ...
  test('zeigt die Optionen des Flows und übernimmt Wert=Label', async () => {
...
describe('ArgumentPicker · Wissensbasis', () => {
  const arg: FlowArgument = { name: 'raum', typ: 'wissensbasis', beschreibung: '', pflicht: true };
  ...
  test('lädt Sammlungen und übernimmt ID als Wert, Name als Label', async () => {
    ...
    expect(apiMock.get).toHaveBeenCalledWith('/flows/sammlungen', expect.anything());
  });
