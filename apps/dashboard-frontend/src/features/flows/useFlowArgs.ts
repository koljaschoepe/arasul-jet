/**
 * useFlowArgs — der Zustand der Argument-Eingabe im Composer (Plan 011, Schritt 14).
 *
 ...
 * Flow-Befehl beginnt. Eine Race beim Start (`begin`) gibt es nicht, weil jede
...
export function useFlowArgs(value: string, onChange: (v: string) => void) {
  ...
  const begin = useCallback(
    (flow: Flow) => {
      // Flows OHNE Argument betreten die Argument-Eingabe gar nicht erst — sonst
      ...
      if (flow.argumente.length === 0) {
        setState(null);
        setPickerFor(null);
        onChange(`/${flow.name} `);
        return;
      }
      const st = beginState(flow);
      setState(st);
      onChange(buildFieldValue(flow.name, st.slots, 0));
      setPickerFor(istPicker(flow.argumente[0]) ? 0 : null);
    },
    [onChange]
  );
  ...
  const reconcile = useCallback(
    (newValue: string): boolean => {
      ...
      if (!r.state) setPickerFor(null);
        ...
        setPickerFor(istPicker(r.state.flow.argumente[r.state.active]) ? r.state.active : null);
      }
      return true;
    },
  ...
  // nicht mehr zum laufenden Flow-Befehl → Eingabe verlassen. In gültiger
  // Argument-Eingabe beginnt der Wert IMMER mit „/<flow> " (buildFieldValue),
  // der Effekt greift dort also nie.
  useEffect(() => {
    if (state && !value.startsWith(`/${state.flow.name}`)) {
      setState(null);
      setPickerFor(null);
    }
  }, [value, state]);

  const ghost = state ? ghostSuffix(state.flow, state.slots, state.active, value) : '';
  const activeArg: FlowArgument | undefined = state
    ? state.flow.argumente[state.active]
    : undefined;
  const pickerArg: FlowArgument | undefined =
    pickerFor != null && state ? state.flow.argumente[pickerFor] : undefined;
  // Gibt es noch ein weiteres Argument, zu dem Tab springen kann?
  const canAdvance = state != null && state.active < state.flow.argumente.length - 1;
