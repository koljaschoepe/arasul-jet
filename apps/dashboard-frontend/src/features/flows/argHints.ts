 ...
 *
 * Nach der Flow-Auswahl steht der Befehl schwarz im Feld, dahinter grau die
 ...
 * Flow-Aufruf gehört. So liegt die knifflige Logik an einer direkt testbaren
 * Stelle; die Komponente zeichnet nur.
 */
import type { Flow, FlowArgument } from '@/types/flows';
...
export function placeholderFor(arg: FlowArgument): string {
  return arg.pflicht ? `<${arg.name}>` : `[${arg.name}]`;
}

/**
 * Baut den Feldwert aus dem Flow-Namen und den bereits gefüllten Argument-Labels.
 ...
 */
export function buildFieldValue(flowName: string, slots: ArgSlot[], active: number): string {
  const labels = slots.slice(0, active).map(s => s.label);
  const activeLabel = slots[active]?.label ?? '';
  const prefix = `/${flowName} `;
...
export function activePrefix(flowName: string, slots: ArgSlot[], active: number): string {
  const labels = slots.slice(0, active).map(s => s.label);
  const committed = labels.join(' ');
  return committed ? `/${flowName} ${committed} ` : `/${flowName} `;
}
 ...
 */
export function ghostSuffix(flow: Flow, slots: ArgSlot[], active: number, value: string): string {
  const args = flow.argumente;
  const teile: string[] = [];
...
// React, ohne DOM. Der Hook (useFlowArgs) ruft sie nur auf und hält das
...
/** Startet die Eingabe für einen frisch gewählten Flow (Argument 0 ist aktiv, leer). */
export function beginState(flow: Flow): ArgState {
  return { flow, slots: [{ ...LEER }], active: 0 };
}

/** Das gerade aktive Argument (oder undefined, wenn der Flow keine (mehr) hat). */
export function activeArgOf(state: ArgState): FlowArgument | undefined {
  return state.flow.argumente[state.active];
}
...
export function reconcileState(
  ...
  const { flow, slots, active } = state;
  const aktiv = flow.argumente[active];
  const prefix = activePrefix(flow.name, slots, active);
  ...
  if (newValue.startsWith(prefix)) {
    ...
    if (istPicker(aktiv)) {
      ...
      return { state, value: buildFieldValue(flow.name, slots, active) };
    }
    const neue = slots.slice();
    neue[active] = { value: rest, label: rest };
    return { state: { flow, slots: neue, active }, value: newValue };
  }
  ...
  // verloren:
  //   0) Der Wert reicht nicht einmal mehr bis zum Befehl selbst („/<flow> ")
  ...
  if (active > 0) {
    ...
    if (istPicker(flow.argumente[newActive])) neue[newActive] = { ...LEER };
    return {
      state: { flow, slots: neue, active: newActive },
      value: buildFieldValue(flow.name, neue, newActive),
    };
  }
...
export function advanceState(state: ArgState): {
  ...
  const { flow, slots, active } = state;
  const newActive = active + 1;
  if (newActive >= flow.argumente.length) {
    // Kein weiteres Argument — Feld unverändert lassen.
    return {
      state,
      value: buildFieldValue(flow.name, slots, active),
      openPicker: false,
      advanced: false,
    };
  }
  ...
  return {
    state: { flow, slots: neue, active: newActive },
    value: buildFieldValue(flow.name, neue, newActive),
    openPicker: istPicker(flow.argumente[newActive]),
    advanced: true,
  };
}
...
export function collectValues(state: ArgState): Record<string, string> {
  ...
  state.flow.argumente.forEach((arg, i) => {
