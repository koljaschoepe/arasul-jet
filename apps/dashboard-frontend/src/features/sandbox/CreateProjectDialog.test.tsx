...
describe('CreateProjectDialog — Netzwerkmodi', () => {
  ...
  test('zeigt alle drei Modus-Optionen an', () => {
    ...
    expect(screen.getByText('Nur Internet')).toBeInTheDocument();
    expect(screen.getByText('Interne Dienste')).toBeInTheDocument();
    expect(screen.getByText('Voller Systemzugriff (Admin)')).toBeInTheDocument();
  });
  ...
  test('Auswahl von Infrastruktur zeigt den Warnhinweis', async () => {
    ...
    await user.click(screen.getByText('Voller Systemzugriff (Admin)'));
