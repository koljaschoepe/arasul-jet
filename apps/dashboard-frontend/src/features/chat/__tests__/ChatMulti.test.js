...
// Mock useApi hook (replaces axios)
const mockApi = {
  get: jest.fn(),
  post: jest.fn(),
  put: jest.fn(),
...
describe('ChatMulti Component', () => {
  ...
  beforeEach(() => {
    ...
    mockApi.get.mockImplementation(url => {
      ...
      if (url.includes('/chats') && !url.includes('/messages') && !url.includes('/jobs')) {
        return Promise.resolve({ chats: mockChats });
      }
      ...
      if (url.includes('/models/installed')) {
        return Promise.resolve({ models: mockModels });
      }
      if (url.includes('/models/default')) {
        return Promise.resolve({ model: { id: 'qwen3:7b' } });
      }
      ...
      if (url.includes('/llm/queue')) {
        return Promise.resolve({ pending_count: 0, processing: null, queue: [] });
      }
  ...
  describe('Chat Creation', () => {
    test('Neuer Chat kann erstellt werden', async () => {
      mockApi.post.mockResolvedValueOnce({
        chat: { id: 3, title: 'Neuer Chat', created_at: new Date().toISOString() },
      });
    ...
    test('Chat-Wechsel lädt neue Nachrichten', async () => {
      ...
      await waitFor(() => {
        expect(mockApi.get).toHaveBeenCalledWith(
          expect.stringContaining('/chats/2/messages'),
          expect.any(Object)
        );
      });
    });
  ...
  describe('Job Reconnection', () => {
    test('kann zu laufendem Job reconnecten', async () => {
      ...
      await waitFor(
        () => {
          const reconnectCalls = mockApi.get.mock.calls.filter(call => call[0].includes('/jobs'));
