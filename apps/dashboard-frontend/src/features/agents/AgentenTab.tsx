...
const FlowsView = lazy(() => import('./FlowsView'));

type Selection = { mode: 'none' } | { mode: 'new' } | { mode: 'edit'; agent: FlowAgent };
type AreaView = 'agents' | 'flows';
...
export default function AgentenTab() {
  ...
  const [view, setView] = useState<AreaView>('agents');
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-background">
      {/* Bereichs-Umschalter: Agenten | Flüsse */}
      <div className="flex items-center gap-1 border-b border-border px-2 py-1.5">
        <button type="button" className={tabCls('agents')} onClick={() => setView('agents')}>
          <Bot className="mr-1 inline h-4 w-4" /> Agenten
        </button>
        <button type="button" className={tabCls('flows')} onClick={() => setView('flows')}>
          <GitBranch className="mr-1 inline h-4 w-4" /> Flüsse
        </button>
      </div>
