/**
 * CellEditor - Consolidated inline cell editor component
 * Single implementation for all grid/table editing in the application.
 *
 * Props:
 *   value       - Current cell value
 *   field       - Field definition (field_type or type, options)
 *   onSave      - Called with (value, direction?) where direction is 'prev'|'next' for Tab
 *   onCancel    - Called on Escape
 *   validate    - Whether to validate before saving (default: true)
 *   classPrefix - CSS class prefix: 'grid' or 'dt' (default: 'grid')
 */
...
const CellEditor = memo(function CellEditor({
  ...
  const handleSave = (val, direction) => {
    if (validate) {
