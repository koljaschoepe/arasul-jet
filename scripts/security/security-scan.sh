...
if [ "$SKIP_NPM" = false ]; then
    ...
    FRONTEND_DIR="$PROJECT_ROOT/apps/dashboard-frontend"
...
OS Hardening Scripts:
  [x] scripts/security/harden-ssh.sh - SSH key-only, port 2222, fail2ban
  [x] scripts/security/setup-firewall.sh - UFW with minimal ports
  [x] scripts/setup/setup-service-user.sh - Dedicated service user
  [x] scripts/util/disable-auto-updates.sh - Stable system config
  [x] scripts/security/harden-os.sh - Orchestrator
