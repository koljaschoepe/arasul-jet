...
#
# Der Mensch, der installiert, und nicht `$HOME` (J35, 25.09.2026): unter
# `sudo` zeigt `$HOME` auf Ubuntu nach /root, die Pruefung fand dort nie einen
# Schluessel und brach ab. Und `AllowUsers` nennt denselben Menschen statt fest
# `arasul` -- auf einem Geraet, dessen Konto anders heisst, hatte die Haertung
# sonst genau den ausgesperrt, der sie gerade laufen liess.
ARASUL_USER="${ARASUL_SSH_USER:-${SUDO_USER:-arasul}}"
ARASUL_HOME="$(getent passwd "$ARASUL_USER" 2>/dev/null | cut -d: -f6)"
ARASUL_HOME="${ARASUL_HOME:-/home/${ARASUL_USER}}"
if [ ! -f "$ARASUL_HOME/.ssh/authorized_keys" ] || [ ! -s "$ARASUL_HOME/.ssh/authorized_keys" ]; then
    echo ""
    echo "WARNING: No SSH authorized_keys found for user '${ARASUL_USER}' (${ARASUL_HOME}/.ssh)!"
    echo "  You MUST add an SSH key before disabling password auth."
    echo "  Run: ssh-copy-id -p 22 arasul@<jetson-ip>"
    echo "  Then re-run this script."
    echo ""
    if [ "$NON_INTERACTIVE" = true ]; then
        echo "SSH-Haertung uebersprungen: ${ARASUL_USER} hat keinen SSH-Schluessel in ${ARASUL_HOME}/.ssh/authorized_keys -- ohne ihn sperrte das Abschalten der Passwort-Anmeldung jeden aus"
        exit 1
    fi
...
# Only allow the installing user
AllowUsers ${ARASUL_USER}
