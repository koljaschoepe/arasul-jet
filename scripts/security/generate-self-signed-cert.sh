...
echo "🔐 Generating Self-Signed Certificate for Arasul Platform"
echo "   Domain: $DOMAIN"
...
# This file is auto-generated by generate-self-signed-cert.sh
...
echo "✅ TLS configuration created: $TLS_CONFIG"
echo ""

# Disable Let's Encrypt in traefik.yml (when using self-signed)
TRAEFIK_CONFIG="$CERT_DIR/../traefik.yml"
if [ -f "$TRAEFIK_CONFIG" ]; then
    echo "📝 Updating Traefik configuration to use self-signed certificate..."
    ...
    echo "✅ Traefik configuration updated (backup saved as traefik.yml.backup)"
fi
...
echo "   - Chrome/Edge: Click 'Advanced' → 'Proceed to arasul.local (unsafe)'"
