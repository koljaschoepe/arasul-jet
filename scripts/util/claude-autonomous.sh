...
- Fuehre aus: ./scripts/util/telegram-notify.sh "BLOCKER: [Beschreibung]"
- Stoppe die Session
...
if [ -x "$PROJECT_DIR/scripts/util/telegram-notify.sh" ]; then
  "$PROJECT_DIR/scripts/util/telegram-notify.sh" "Claude Autonomous Session gestartet" 2>/dev/null || true
fi
