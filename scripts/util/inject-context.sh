...
while IFS= read -r file; do
  ...
  # Backend routes
  if [[ "$file" == apps/dashboard-backend/src/routes/* ]]; then
  ...
  # Backend services
  if [[ "$file" == apps/dashboard-backend/src/services/* ]]; then
  ...
  # Frontend components
  if [[ "$file" == apps/dashboard-frontend/src/components/* ]]; then
  ...
  # Frontend CSS
  if [[ "$file" == *.css ]] && [[ "$file" == apps/dashboard-frontend/* ]]; then
    if [[ ! " ${CONTEXTS[*]} " =~ " component " ]]; then
      CONTEXTS+=("component")
