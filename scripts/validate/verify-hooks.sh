...
for script in ./scripts/test/run-tests.sh ./scripts/util/telegram-notify.sh ./scripts/test/run-typecheck.sh; do
  if [ -f "$script" ]; then
    if [ -x "$script" ]; then
...
for script in ./scripts/test/run-tests.sh ./scripts/util/telegram-notify.sh; do
  if [ -f "$script" ]; then
...
if [ -f "./scripts/test/run-tests.sh" ]; then
  if grep -q "stop_hook_active" "./scripts/test/run-tests.sh"; then
