...
import {
    FiX, FiSave, FiPlus, FiTrash2, FiDownload, FiRefreshCw,
    FiCheck, FiAlertCircle, FiChevronUp, FiChevronDown, FiColumns,
    FiMoreVertical, FiEdit2, FiType, FiCopy, FiClipboard, FiScissors,
    FiCornerUpLeft, FiCornerUpRight, FiChevronsLeft, FiChevronsRight,
    FiChevronLeft, FiChevronRight, FiUpload, FiDatabase
} from 'react-icons/fi';
...
const ColumnMenu = memo(function ColumnMenu({
    ...
    const handleDelete = async () => {
        if (!window.confirm(`Spalte "${field.name}" wirklich löschen? Alle Daten in dieser Spalte gehen verloren.`)) {
            return;
        }
...
function DataTableEditor({ tableSlug, tableName, onClose, onSave }) {
    ...
    useEffect(() => {
        const checkIndexStatus = async () => {
            try {
                const response = await axios.get(`${API_BASE}/v1/datentabellen/tables/${tableSlug}/index/status`);
    ...
    const handleCellSave = async (rowId, fieldSlug, value, direction, skipUndo = false) => {
        ...
        try {
            ...
            setRows(prev => prev.map(row =>
                row._id === rowId ? { ...row, ...updatedRow } : row
            ));
            ...
            if (err.response?.status === 409) {
                setError('Konflikt: Ein anderer Benutzer hat diesen Datensatz geändert. Daten werden neu geladen.');
                setSaveStatus('error');
    ...
    const handleDeleteSelected = async () => {
        ...
        if (!window.confirm(`${selectedRows.size} Zeile(n) wirklich löschen?`)) return;
    ...
    }, [activeCell, editingCell, rows, fields, handleCopy, handleCut, handlePaste, handleUndo, handleRedo]);
            ...
            <div className="yaml-editor-container" style={{ maxWidth: '1600px' }}>
                ...
                <div className="yaml-editor-header">
                    <div className="yaml-editor-title">
                        ...
                        {table?.description && (
                            <p className="yaml-editor-description">{table.description}</p>
                        )}
                    </div>
                    ...
                    <div className="yaml-editor-actions">
                        ...
                        <button
                            className="yaml-btn yaml-btn-secondary"
                            onClick={handleClose}
                        >
                            <FiX /> Schließen
                        </button>
                                    ...
                                    >
                                        ...
                                        {fields.map((field, colIdx) => {
                                            const isEditing = editingCell?.rowId === row._id &&
                                                              editingCell?.fieldSlug === field.slug;
