...
import {
    FiX, FiPlus, FiTrash2, FiDownload, FiRefreshCw,
    FiCheck, FiAlertCircle, FiChevronUp, FiChevronDown,
    FiMoreVertical, FiEdit2, FiType, FiCopy, FiClipboard, FiScissors,
    FiCornerUpLeft, FiCornerUpRight, FiChevronsLeft, FiChevronsRight,
    FiChevronLeft, FiChevronRight, FiArrowLeft, FiMessageSquare, FiSend,
    FiCode, FiFilter
} from 'react-icons/fi';
 ...
 */
const InlineColumnCreator = memo(function InlineColumnCreator({ tableSlug, onColumnAdded, existingSlugs }) {
...
const ColumnMenu = memo(function ColumnMenu({
    ...
    const handleDelete = async () => {
        if (!window.confirm(`Spalte "${field.name}" wirklich löschen?`)) return;
...
const CellContextMenu = memo(function CellContextMenu({
    position, onClose, onCopy, onPaste, onCut, onDelete, hasClipboard
}) {
    const menuRef = useRef(null);
    ...
    return (
        <div className="excel-context-menu" ref={menuRef} style={{ top: position.y, left: position.x }}>
            <button onClick={onCopy}><FiCopy /> Kopieren <span>Strg+C</span></button>
            <button onClick={onCut}><FiScissors /> Ausschneiden <span>Strg+X</span></button>
            <button onClick={onPaste} disabled={!hasClipboard}><FiClipboard /> Einfügen <span>Strg+V</span></button>
            <div className="excel-menu-divider" />
            <button onClick={onDelete}><FiTrash2 /> Löschen <span>Entf</span></button>
        </div>
    );
});
            ...
            {result && (
                <div className="excel-ai-result">
                    ...
                    <div className="excel-ai-stats">
                        <span>{result.rowCount} Ergebnis{result.rowCount !== 1 ? 'se' : ''}</span>
                        {result.rowCount > 0 && (
                            <button
                                className="excel-ai-apply"
                    ...
                    {result.results && result.results.length > 0 && (
                        <div className="excel-ai-preview">
                            <table>
                                <thead>
                                    <tr>
                                        {Object.keys(result.results[0]).slice(0, 5).map(key => (
                                            <th key={key}>{key}</th>
                                        ))}
                            ...
                            {result.results.length > 5 && (
                                <div className="excel-ai-more">
                                    ... und {result.results.length - 5} weitere
                                </div>
                            )}
                        </div>
                    )}
...
function ExcelEditor({ tableSlug, tableName, onClose }) {
    ...
    const moveToCell = useCallback((direction) => {
    ...
    const handleGhostRowEdit = useCallback(async (fieldSlug, value) => {
    ...
    // Update cell value
    const handleCellSave = useCallback(async (rowId, fieldSlug, value, direction, skipUndo = false) => {
        setEditingCell(null);
        ...
        try {
            setSaving(true);
            const response = await axios.patch(`${API_BASE}/v1/datentabellen/tables/${tableSlug}/rows/${rowId}`, {
                [fieldSlug]: value,
                _expected_updated_at: oldRow?._updated_at
            });

            setRows(prev => prev.map(row =>
                row._id === rowId ? { ...row, ...response.data.data } : row
            ));

            if (!skipUndo) {
                setUndoStack(prev => [...prev.slice(-MAX_UNDO_HISTORY + 1), { rowId, fieldSlug, oldValue, newValue: value }]);
                setRedoStack([]);
            }
    ...
    const handleDeleteSelected = async () => {
        ...
        if (!window.confirm(`${selectedRows.size} Zeile(n) löschen?`)) return;
    ...
    const handleKeyDown = useCallback((e) => {
        if (e.ctrlKey || e.metaKey) {
            switch (e.key.toLowerCase()) {
                case 'c': if (!editingCell) { e.preventDefault(); handleCopy(); } return;
                case 'x': if (!editingCell) { e.preventDefault(); handleCut(); } return;
                case 'v': if (!editingCell) { e.preventDefault(); handlePaste(); } return;
                case 'z':
                    e.preventDefault();
        ...
        switch (e.key) {
            case 'ArrowUp': e.preventDefault(); if (row > 0) setActiveCell({ row: row - 1, col }); break;
            case 'ArrowDown': e.preventDefault(); if (row < numRows - 1) setActiveCell({ row: row + 1, col }); break;
            case 'ArrowLeft': e.preventDefault(); if (col > 0) setActiveCell({ row, col: col - 1 }); break;
            case 'ArrowRight': e.preventDefault(); if (col < numCols - 1) setActiveCell({ row, col: col + 1 }); break;
            case 'Tab': e.preventDefault(); moveToCell(e.shiftKey ? 'prev' : 'next'); break;
            case 'Enter':
            case 'F2':
                e.preventDefault();
                if (displayRows[row] && fields[col]) {
    ...
    }, [activeCell, editingCell, displayRows, fields, handleCopy, handleCut, handlePaste, handleUndo, handleRedo, moveToCell, handleCellSave]);
    ...
    const formatValue = (value, fieldType) => {
        ...
        switch (fieldType) {
            ...
            case 'currency': return new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(value);
            case 'number': return new Intl.NumberFormat('de-DE').format(value);
            case 'date': return new Date(value).toLocaleDateString('de-DE');
            case 'datetime': return new Date(value).toLocaleString('de-DE');
    ...
    return (
        <div className="excel-fullscreen">
            ...
            <header className="excel-header">
                ...
                <div className="excel-header-center">
                    <div className={`excel-save-status ${saveStatus || ''}`}>
                        {saving && <><FiRefreshCw className="spin" /> Speichere...</>}
                        {saveStatus === 'success' && <><FiCheck /> Gespeichert</>}
                        {saveStatus === 'error' && <><FiAlertCircle /> Fehler</>}
                    </div>
                </div>

                <div className="excel-header-right">
                    <span className="excel-stats">{totalRows} Zeilen, {fields.length} Spalten</span>
                </div>
            </header>
            ...
            <div className="excel-toolbar">
                ...
                <button className="excel-btn excel-btn-icon" onClick={handleUndo} disabled={undoStack.length === 0} title="Rückgängig (Strg+Z)">
                    <FiCornerUpLeft />
                </button>
                <button className="excel-btn excel-btn-icon" onClick={handleRedo} disabled={redoStack.length === 0} title="Wiederholen (Strg+Y)">
                    <FiCornerUpRight />
                </button>

                <div className="excel-toolbar-divider" />

                <button className="excel-btn" onClick={handleExportCSV} disabled={saving || rows.length === 0}>
                    <FiDownload /> Export
                </button>
                            ...
                            {fields.map((field) => (
                                ...
                                >
                                    <div className="excel-th-content">
                                        ...
                                        <button
                                            ...
                                            onClick={(e) => {
                                                ...
                                                setColumnMenu({ field, position: { top: rect.bottom + 4, left: rect.left } });
                                            }}
                                        >
                                            <FiMoreVertical />
                                        </button>
                                    ...
                                    <div
                                        ...
                                        onMouseDown={(e) => handleResizeStart(e, field.slug, columnWidths[field.slug] || 150)}
                                    />
                                </th>
                            ))}
                            <InlineColumnCreator
            ...
            <div className="excel-pagination">
                <div className="excel-pagination-info">
                    {totalRows > 0 ? `${((page - 1) * pageSize) + 1}–${Math.min(page * pageSize, totalRows)} von ${totalRows}` : '0 Einträge'}
                </div>

                <div className="excel-pagination-controls">
                    <button disabled={page === 1} onClick={() => setPage(1)}><FiChevronsLeft /></button>
                    ...
                    <button disabled={page >= totalPages} onClick={() => setPage(totalPages)}><FiChevronsRight /></button>
                </div>

                <div className="excel-pagination-size">
                    <select value={pageSize} onChange={(e) => { setPageSize(+e.target.value); setPage(1); }}>
                        {PAGE_SIZE_OPTIONS.map(s => <option key={s} value={s}>{s} Zeilen</option>)}
                    </select>
                </div>
            </div>
