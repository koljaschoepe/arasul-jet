 ...
 */
router.post('/zero-config/init', requireAuth, apiLimiter, async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: 'Setup konnte nicht gestartet werden',
 ...
 */
router.post('/zero-config/token', requireAuth, apiLimiter, async (req, res) => {
    try {
        ...
        if (!setupToken || !botToken) {
            return res.status(400).json({
                error: 'Setup-Token und Bot-Token sind erforderlich',
                timestamp: new Date().toISOString()
            });
        }
        ...
        if (session.rows.length === 0) {
            return res.status(404).json({
                error: 'Setup-Session nicht gefunden oder abgelaufen',
                timestamp: new Date().toISOString()
            });
        }
 ...
 */
router.get('/zero-config/status/:token', requireAuth, async (req, res) => {
    try {
 ...
 */
router.post('/zero-config/chat-detected', async (req, res) => {
    try {
        ...
        if (session.rows.length === 0) {
            return res.status(404).json({
                error: 'Session nicht gefunden oder nicht im Warte-Status',
                timestamp: new Date().toISOString()
            });
        }
        ...
        res.status(500).json({
            error: 'Setup konnte nicht abgeschlossen werden',
            message: error.message,
            timestamp: new Date().toISOString()
        });
    }
 ...
 */
router.post('/zero-config/complete', requireAuth, apiLimiter, async (req, res) => {
    try {
        ...
        if (session.rows.length === 0) {
            return res.status(404).json({
                error: 'Abgeschlossene Session nicht gefunden',
                timestamp: new Date().toISOString()
            });
        }
        ...
        logger.error(`Error finalizing setup: ${error.message}`);
        res.status(500).json({
            error: 'Setup konnte nicht finalisiert werden',
            message: error.message,
            timestamp: new Date().toISOString()
        });
    }
 ...
 */
router.post('/rules', requireAuth, apiLimiter, async (req, res) => {
    try {
        const userId = req.user.id;
        const {
            name,
        ...
        if (!name || !eventSource || !eventType || !messageTemplate) {
            return res.status(400).json({
                error: 'Name, Event-Quelle, Event-Typ und Nachrichtenvorlage sind erforderlich',
                timestamp: new Date().toISOString()
            });
        }
 ...
 */
router.post('/rules/:id/test', requireAuth, apiLimiter, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        ...
        if (ruleResult.rows.length === 0) {
            return res.status(404).json({
                error: 'Regel nicht gefunden oder Bot nicht konfiguriert',
                timestamp: new Date().toISOString()
            });
        }
        ...
        logger.error(`Error sending test: ${error.message}`);
        res.status(500).json({
            error: 'Test-Nachricht konnte nicht gesendet werden',
            message: error.message,
            timestamp: new Date().toISOString()
        });
    }
 ...
 */
router.get('/orchestrator/status', requireAuth, async (req, res) => {
    try {
        const result = await db.query(`
        ...
        logger.error(`Error fetching orchestrator status: ${error.message}`);
        res.status(500).json({
            error: 'Orchestrator-Status konnte nicht abgerufen werden',
            timestamp: new Date().toISOString()
        });
    }
});
 ...
 */
router.get('/orchestrator/thinking/:agentType', requireAuth, async (req, res) => {
    try {
        ...
        logger.error(`Error fetching thinking log: ${error.message}`);
        res.status(500).json({
            error: 'Thinking-Log konnte nicht abgerufen werden',
            timestamp: new Date().toISOString()
        });
    }
});
