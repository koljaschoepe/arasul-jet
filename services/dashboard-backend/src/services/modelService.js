...
class ModelService {
    ...
    async deleteModel(modelId) {
        ...
        await db.query('DELETE FROM llm_installed_models WHERE id = $1', [modelId]);
    ...
    async getStatus() {
        const [loaded, installedResult, queueByModelResult, switchStatsResult] = await Promise.all([
            this.getLoadedModel(),
            db.query('SELECT COUNT(*) as count FROM llm_installed_models WHERE status = $1', ['available']),
            db.query('SELECT * FROM get_queue_status_by_model()'),
            db.query(`
                SELECT
