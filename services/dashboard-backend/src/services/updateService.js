...
/**
 * Helper: Run a command with stdin from a file (replaces exec with shell redirection)
 */
...
class UpdateService {
  ...
  async verifySignature(updateFilePath, signatureFilePath) {
    try {
      const publicKeyPath =
        process.env.UPDATE_PUBLIC_KEY_PATH || '/arasul/config/public_update_key.pem';
