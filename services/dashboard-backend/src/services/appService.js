...
const NETWORK_NAME = process.env.DOCKER_NETWORK || 'arasul-jet_arasul-net';
...
class AppService {
    ...
    async getAllApps(filters = {}) {
        ...
        const installations = await db.query(
            'SELECT * FROM app_installations ORDER BY app_id'
        );
    ...
    async installApp(appId, config = {}) {
        ...
        const existing = await db.query(
            'SELECT status FROM app_installations WHERE app_id = $1',
            [appId]
        );
        ...
        try {
            ...
            if (manifest.docker.buildRequired) {
                ...
                if (!imageExists) {
                    throw new Error(`Lokales Image ${manifest.docker.image} nicht gefunden. Bitte zuerst mit 'docker build' erstellen.`);
                }
    ...
    buildTraefikLabels(manifest) {
        ...
        if (manifest.traefik?.enabled !== false) {
            ...
            labels[`traefik.http.routers.${routerName}.priority`] = String(manifest.traefik?.priority || 50);
            ...
            if (manifest.docker.ports?.internal) {
                labels[`traefik.http.services.${routerName}.loadbalancer.server.port`] =
                    String(manifest.docker.ports.internal);
            }
        } else {
        ...
        try {
            ...
            for (const row of result.rows) {
                ...
                if (row.is_secret && row.config_value) {
                    ...
                    config[row.config_key] = val.length > 4
                        ? '****' + val.slice(-4)
                        : '****';
