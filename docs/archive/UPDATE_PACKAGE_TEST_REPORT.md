...
```bash
bash scripts/deploy/create_update_package.sh 2.0.3-test unknown-component postgres
```
...
PRIVATE_KEY_PATH=/nonexistent/key.pem \
  bash scripts/deploy/create_update_package.sh 2.0.5-test postgres
```
...
**Script**: `scripts/util/arasul-usb-trigger.sh`
...
./scripts/deploy/create_update_package.sh test-1.0.0 postgres || exit 1
...
./scripts/deploy/create_update_package.sh test-1.0.1 invalid postgres || exit 1

# Test 4: Missing key handling
PRIVATE_KEY_PATH=/tmp/nonexistent.pem \
  ./scripts/deploy/create_update_package.sh test-1.0.2 postgres 2>&1 | \
...
- `scripts/deploy/create_update_package.sh` (221 lines)
- `scripts/deploy/sign_update_package.py` (198 lines)
