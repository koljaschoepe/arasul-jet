...
```bash
# Standard workflow
./scripts/test/run-tests.sh --backend   # Run backend tests
...
1. Create in `apps/dashboard-frontend/src/components/`
...
- **Benachrichtigen** - `./scripts/util/telegram-notify.sh "BLOCKER: [Beschreibung]"`
...
```bash
./scripts/util/claude-autonomous.sh           # Hintergrund-Session
./scripts/util/claude-autonomous.sh --attach  # Mit Attach
./scripts/util/claude-autonomous.sh --status  # Status prüfen
./scripts/util/claude-autonomous.sh --kill    # Session beenden
```
...
2. Run ./scripts/test/run-tests.sh --backend before commits
...
- Entry: `apps/dashboard-frontend/src/App.js`
- Components: `src/components/` (20 files)
- Colors: Primary #45ADFF, Background #101923/#1A2330

**Backend Tasks:**

- Entry: `apps/dashboard-backend/src/index.js`
