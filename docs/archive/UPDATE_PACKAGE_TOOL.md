...
```bash
# Syntax
./scripts/deploy/create_update_package.sh <version> <component1> [component2...]

# Examples
./scripts/deploy/create_update_package.sh 2.1.0 dashboard-backend
./scripts/deploy/create_update_package.sh 2.2.0 dashboard-backend dashboard-frontend
./scripts/deploy/create_update_package.sh 2.3.0 postgres n8n
```
...
**Location**: `scripts/deploy/create_update_package.sh`
...
**Location**: `scripts/deploy/sign_update_package.py`
...
```bash
# Sign package
python3 scripts/deploy/sign_update_package.py <package.tar.gz> <private_key.pem>

# Verify signature
python3 scripts/deploy/sign_update_package.py --verify <package.araupdate> <public_key.pem>
```
...
./scripts/deploy/create_update_package.sh 2.0.1-test postgres
...
```bash
# Create test package
./scripts/deploy/create_update_package.sh 2.0.2-test postgres
...
./scripts/deploy/create_update_package.sh ...
```
...
# Then create package
cd ../../../../
./scripts/deploy/create_update_package.sh 2.1.0 n8n
```
...
2. **Build Package**
   ...
   ./scripts/deploy/create_update_package.sh 2.1.0 \
       dashboard-backend \
       dashboard-frontend \
       llm-service \
       embedding-service \
...
USB trigger script: `scripts/util/arasul-usb-trigger.sh`
