...
- [ ] `config/traefik/dynamic/middlewares.yml` Lines 218, 231 - PLACEHOLDER-Credentials durch generierte Hashes ersetzen (bei Deployment: `scripts/security/generate_htpasswd.sh`)
- [x] Pre-Deployment-Check in `scripts/validate/validate_config.sh` hinzufuegen der PLACEHOLDER-Werte erkennt
...
- [x] `scripts/validate/validate_config.sh`: Passwort-Staerke-Anforderung von 4 auf mindestens 12 Zeichen erhoeht + `arasul123` in Blocklist
...
- [x] `scripts/setup/preconfigure.sh` erstellt (8 Schritte, idempotent):
...
- [x] `scripts/deploy/create_update_package.sh` ueberarbeitet:
...
- [x] `scripts/security/harden-ssh.sh` erstellt:
...
- [x] `scripts/security/setup-firewall.sh` erstellt:
...
- [x] `scripts/setup/setup-service-user.sh` erstellt:
...
- [x] `scripts/util/disable-auto-updates.sh` erstellt:
...
- [x] `scripts/security/security-scan.sh` erstellt:
...
- [x] `scripts/security/harden-os.sh` Orchestrator erstellt (ruft alle Scripts sequenziell)
...
- [x] Clean-Install auf frischem Jetson testen
  - `scripts/test/integration-test.sh` - Automatisierter Integrationstest (12 Bereiche: Services, Auth, System, LLM, RAG, Settings, Datentabellen, Metrics, Logs, Services-Mgmt, Update, Backup)
...
- [x] Startup-Zeit messen (Cold Boot bis UI erreichbar)
  - `scripts/test/measure-performance.sh` - Automatisierte Performance-Messung
...
- [x] Pre-Shipping-Checkliste:
  ...
  - `scripts/deploy/verify-deployment.sh` - Automatisierte Pruefung mit --fix Option
  - [x] Alle Tests gruen
...
- [x] `scripts/deploy/create-deployment-image.sh`:
...
- [x] Log-Export: `scripts/util/export-support-logs.sh` (anonymisiert, keine Passwoerter)
...
1. Tests laufen lassen (`./scripts/test/run-tests.sh`)
