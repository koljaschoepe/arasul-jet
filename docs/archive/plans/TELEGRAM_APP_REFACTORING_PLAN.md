...
**Datei:** `/apps/dashboard-backend/src/routes/telegramApp.js`
...
**Datei:** `/apps/dashboard-backend/src/services/telegramWebhookService.js`
...
**Neue Datei:** `/apps/dashboard-backend/src/services/telegramWebSocketService.js`
...
class TelegramWebSocketService {
  ...
  broadcast(setupToken, data) {
    ...
    logger.info(
      `Broadcast to ${clients.size} client(s) for token ${setupToken.substring(0, 8)}...`
    );
  }
...
**Änderung in:** `/apps/dashboard-backend/src/index.js`
...
**Datei:** `/apps/dashboard-backend/src/routes/telegramBots.js`
...
router.post('/webhook/:botId/:secret', async (req, res) => {
  ...
  logger.info(`Webhook received for bot ${botId}`, {
    ...
    messageType: req.body?.message?.text
      ? 'text'
...
**Neue Datei:** `/apps/dashboard-backend/src/services/telegramAppService.js`
...
**Änderung in:** `/apps/dashboard-backend/src/routes/telegramApp.js`
...
**Datei:** `/apps/dashboard-frontend/src/App.js`
...
**Neue Datei:** `/apps/dashboard-frontend/src/components/TelegramAppModal.js`
...
function TelegramAppModal({ isOpen, onClose }) {
  ...
  const handleDeleteBot = async botId => {
    if (
      !window.confirm('Bot wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.')
    ) {
      return;
    }
...
**Neue Datei:** `/apps/dashboard-frontend/src/components/TelegramAppModal.css`
...
**Änderung in:** `/apps/dashboard-frontend/src/components/TelegramBots/BotSetupWizard.js`
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        setErrorDetails(
          'Bitte überprüfe den Token. Er sollte das Format "123456789:ABCdefGHI..." haben.'
        );
...
**Neue Datei:** `/apps/dashboard-backend/__tests__/unit/telegramApp.test.js`
...
**Neue Datei:** `/apps/dashboard-backend/__tests__/integration/telegramSetup.test.js`
...
| Datei                                                                 | Beschreibung                           |
| --------------------------------------------------------------------- | -------------------------------------- |
| `/apps/dashboard-backend/src/services/telegramWebSocketService.js`    | WebSocket Server für Real-time Updates |
| `/apps/dashboard-backend/src/services/telegramAppService.js`          | App-Lifecycle Service                  |
| `/services/postgres/init/034_telegram_app_schema.sql`                 | Neues DB-Schema                        |
| `/apps/dashboard-frontend/src/components/TelegramAppModal.js`         | Haupt-Modal Komponente                 |
| `/apps/dashboard-frontend/src/components/TelegramAppModal.css`        | Modal Styles                           |
| `/apps/dashboard-backend/__tests__/unit/telegramApp.test.js`          | Unit Tests                             |
| `/apps/dashboard-backend/__tests__/integration/telegramSetup.test.js` | Integration Tests                      |

### Geänderte Dateien

| Datei                                                                    | Änderung                                  |
| ------------------------------------------------------------------------ | ----------------------------------------- |
| `/apps/dashboard-backend/src/index.js`                                   | WebSocket Service initialisieren          |
| `/apps/dashboard-backend/src/routes/telegramApp.js`                      | Neue Endpunkte, WebSocket Integration     |
| `/apps/dashboard-backend/src/routes/telegramBots.js`                     | Verbessertes Logging                      |
| `/apps/dashboard-backend/src/services/telegramWebhookService.js`         | Null-Validierung, Setup-Notification      |
| `/apps/dashboard-frontend/src/App.js`                                    | Sidebar-Eintrag entfernen, Dashboard-Icon |
| `/apps/dashboard-frontend/src/components/TelegramBots/BotSetupWizard.js` | WebSocket Integration                     |

### Zu löschende/deprecate Dateien

| Datei                                                       | Grund                              |
| ----------------------------------------------------------- | ---------------------------------- |
| `/apps/dashboard-frontend/src/components/TelegramBotApp.js` | Durch TelegramAppModal ersetzt     |
| `/apps/dashboard-frontend/src/telegram-bot-app.css`         | Durch TelegramAppModal.css ersetzt |
...
_Dieser Plan wurde erstellt basierend auf einer umfassenden Codebase-Analyse mit 8 spezialisierten Agenten._
