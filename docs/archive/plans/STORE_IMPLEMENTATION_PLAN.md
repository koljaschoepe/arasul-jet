...
<Link to="/store" className={isActive('/store')}>
  <FiPackage />
  <span>Store</span>
  {downloadCount > 0 && <span className="download-badge">{downloadCount}</span>}
</Link>
```
...
_Erstellt am: 2026-02-07_
_Basierend auf Benutzer-Interview_
_Status: Bereit zur Implementierung_
