...
| GPU_VORRANG                          | true                         | Ob das Backend dem Indexer meldet, dass es die GPU fuer einen Nutzerlauf haelt (Fund 22.08.2026). Auf `false` gesetzt rechnen beide wieder gegeneinander, und der Nutzer wartet je Chat-Runde 30 bis 60 Sekunden auf einen Modellwechsel                                                                                                                                                        |
| GPU_VORRANG_FRIST_S                  | 30                           | Wie lange eine einzelne Vorrang-Meldung gilt. Sie wird waehrend des Laufs alle FRIST/3 Sekunden erneuert. Eine Frist statt eines Schalters: faellt das Backend aus, arbeitet der Indexer von selbst weiter                                                                                                                                                                                      |
| GPU_VORRANG_ZEITLIMIT_MS             | 1500                         | Zeitlimit fuer die Meldung an den Indexer. Ein Indexer, der nicht antwortet, darf den Chat nicht bremsen                                                                                                                                                                                                                                                                                        |
| GPU_VORRANG_HOECHSTFRIST_S           | 120                          | Im Indexer: laenger als das haelt er sich nie zurueck, egal was gemeldet wird. Eine Zusage, die ihn eine Viertelstunde stilllegt, waere ein Ausfall, kein Vorrang                                                                                                                                                                                                                               |
| DOCUMENT_INDEXER_INTERVAL            | 30                           | Scan interval (seconds)                                                                                                                                                                                                                                                                                                                                                                         |
| DOCUMENT_INDEXER_MAX_DOCS_PER_CYCLE  | 10                           | Hoechstzahl Dokumente je Scan-Zyklus. Der Deckel haelt einen Zyklus ueberschaubar                                                                                                                                                                                                                                                                                                               |
| DOCUMENT_INDEXER_NACHBRENNER         | 2                            | Pause in Sekunden, wenn im letzten Zyklus Arbeit liegen blieb (Plan 023 G4). Ohne sie schlief der Indexer dreissig Sekunden, obwohl noch neunzig Dokumente warteten                                                                                                                                                                                                                             |
| INDEXER_WATCHDOG_INTERVAL_SECONDS    | 300                          | Periodic recover_stuck_processing interval (s)                                                                                                                                                                                                                                                                                                                                                  |
| PARTIAL_REPICKUP_INTERVAL_SECONDS    | 3600                         | Takt, in dem unvollständig (`partial`) indexierte Dokumente wieder aufgenommen werden; `0` schaltet es ab (Plan 012 Phase F)                                                                                                                                                                                                                                                                    |
| PARTIAL_REPICKUP_MAX_ATTEMPTS        | 2                            | Wie oft ein einzelnes `partial`-Dokument insgesamt wieder aufgenommen wird (über `retry_count` gezählt)                                                                                                                                                                                                                                                                                         |
| PARTIAL_REPICKUP_BATCH               | 5                            | Wie viele Dokumente ein Wiederaufnahme-Durchlauf höchstens anfasst                                                                                                                                                                                                                                                                                                                              |
...
| RAG_VEKTOR_SUCHE                     | false                        | Plan 021 (agentic RAG): schaltet den semantischen Vektor-Zweig des Agenten-Werkzeugs `rag_suche` ein. Standard **aus** — ohne `dateiname` verweist `rag_suche` auf `dateien_suchen` (grep). Bleibt bis Schritt 8 als Rückfallschalter. Betrifft nur das Flow-/Agenten-Werkzeug, nicht die klassische `/api/rag`-Route.                                                                          |
| INDEXER_EMBEDDING_ENABLED            | true                         | Plan 021 (Schritt 8): Ist `false`, indexiert der document-indexer NUR den Textlayer (`document_chunks`) — kein Embedding, kein Qdrant-Upsert. Der agentische Pfad (`rag_suche` mit `dateiname` → `ladeDokumentText`) liest den Textlayer unabhängig davon weiter. Wird auf `false` gesetzt, wenn `qdrant`/`embedding-service` abgeschaltet werden. Standard **true** = unverändertes Verhalten. |
...
| SANDBOX_HOST_PROJECTS_DIR          | (auto-detected)                     | Optionaler Override: Host-Pfad von `data/projects` (Projektablage) für den rw-Mount `/workspace/projekt` einer an ein Projekt angeschlossenen Sandbox (`sandbox_projects.project_id`, Migration 125). Gleiche Auflösungs-Reihenfolge wie SANDBOX_HOST_DATA_DIR: Env → Inspektion der eigenen Backend-Mounts                                                                                             |
| SANDBOX_DEV_TEMPLATES_DIR          | /arasul/sandbox-build/dev-templates | Vorlagen für die Erweiterungs-Werkstatt (`ANLEITUNG.md`, Beispiel-App/-Flow/-Tool). Read-only aus `services/sandbox/` gemountet; beim Anlegen einer Werkstatt-Sandbox in deren Ordner kopiert (Plan 012 Phase E)                                                                                                                                                                                        |
| EXTENSIONS_DIR                     | /arasul/extensions                  | Ablageort der Erweiterungs-Pakete (je ein Ordner mit `manifest.json` + Assets). Bind-Mount `data/extensions`, überlebt Rebuilds und wandert ins Backup (Plan 012 Phase E)                                                                                                                                                                                                                               |
| EXTENSIONS_WATCH_INTERVAL_MS       | 15000                               | Takt des Werkstatt-Watchers (`services/extensions/werkstattWatcher.js`): scannt die Ordner aller Erweiterungs-Werkstätten und registriert/aktualisiert neue bzw. geänderte Erweiterungen automatisch („Automatisch live", 2026-07-29). `enabled` wird dabei nie verändert                                                                                                                               |
| EXTENSIONS_BRUECKE_ENABLED         | true                                | Notaus der KI-Brücke (Plan 017 Schritt 2): `false` schaltet ALLE `/api/extensions/:id/bruecke/*`-Routen sofort auf 503 — ohne Redeploy der Erweiterungen. Jeder andere Wert lässt die Brücke an                                                                                                                                                                                                         |
| EXTENSIONS_BRUECKE_TOKEN_TTL_MS    | 900000                              | Lebensdauer eines Brücken-Tokens (ms, Standard 15 min). Tokens leben nur im Speicher; die App holt sich über die Dashboard-Seite (postMessage) automatisch frische                                                                                                                                                                                                                                      |
| SELFHEAL_COMPOSE_PROJECT           | (leer)                              | Compose-Projekt, dessen Container die Selbstheilung überwacht. Leer heißt: der Agent liest es aus seinem eigenen Container, was auch bei umbenanntem Projekt stimmt. Ohne diesen Filter überwacht er JEDEN Container des Hosts. Auf dem Orin waren das auch der Prüfstand und die Sandbox-Terminals: 311 CRITICAL-Ereignisse in sieben Tagen zu einem Dienst, den es im Produkt nicht gibt (23.08.2026) |
| SELFHEAL_WARTUNGSDATEI             | /arasul/logs/wartung.aktiv          | Wartungsfenster. Ist diese Datei frisch, läuft ein Deploy, und Kategorie A greift nicht ein. `deploy-local.sh` fasst sie während des ganzen Deploys immer wieder an, es ist also ein Herzschlag und kein Schalter. Ohne sie startete die Selbstheilung Dienste mitten im Deploy neu, gegen den Deploy (23.08.2026, viermal auf dem Orin)                                                                |
...
| SELFHEAL_WARTUNG_NACHLAUF_SEKUNDEN | 60                                  | Nachlauf nach dem Ende der Wartung. Ohne ihn war der Schutz um Sekunden zu kurz: Am 24.08.2026 schloss das Fenster um 00:05:33, und um 00:05:43 startete die Selbstheilung n8n neu. `compose up` war fertig, die Dienste aber noch ungesund und brauchten erst einen erfolgreichen Healthcheck (bei n8n alle 15 s)                                                                                      |
...
| EXTENSIONS_APP_TOKEN_TTL_MS        | 900000                              | Lebensdauer eines Lese-Tokens für die Paket-Dateien einer App-Erweiterung (ms, Standard 15 min). Der Token steht im Pfad des iframes, weil aus dessen opakem Origin kein `SameSite=Strict`-Cookie mitkommt; ohne ihn lädt keine Unterdatei. Nur im Speicher, wird beim Öffnen eines Tabs frisch geholt                                                                                                  |
| EXTENSIONS_NETZ_TIMEOUT_MS         | 20000                               | Plan 023 H1: Zeitlimit eines ausgehenden Aufrufs einer Erweiterung (`POST /api/extensions/:id/bruecke/netz`). Danach bricht der Aufruf ab und meldet das dem Aufrufer; die Erweiterung kann es erneut versuchen                                                                                                                                                                                         |
| EXTENSIONS_NETZ_MAX_ANTWORT        | 1048576                             | Plan 023 H1: Höchstlänge der durchgereichten Antwort in Zeichen (Standard 1 MB). Darüber wird gekürzt und `gekuerzt: true` gesetzt, damit die Erweiterung nicht mit einem halben JSON weiterrechnet, ohne es zu merken                                                                                                                                                                                  |
| EXTENSIONS_ZEITPLAN_TAKT_MS        | 60000                               | Plan 023 H1: wie oft nachgesehen wird, ob ein Zeitplan einer Erweiterung fällig ist. Eine Minute reicht für Uhrzeiten in `HH:MM`; kürzer bringt nichts, länger verpasst das Nachholfenster                                                                                                                                                                                                              |
| EXTENSIONS_ZEITPLAN_NACHHOLEN_MIN  | 10                                  | Plan 023 H1: wie viele Minuten ein verpasster Lauf noch nachgeholt wird. Ohne dieses Fenster hätte ein Gerät, das um 03:00 gerade neu startet, genau diesen einen nächtlichen Lauf verloren                                                                                                                                                                                                             |
...
| CLAUDE_CLI_VERSION                 | stable                              | Version/dist-tag von `@anthropic-ai/claude-code`, die `services/sandbox/claude.sh` beim Erststart installiert. Bewusst **nicht** `latest` — eine brandneue CLI kann einen neuen OAuth-/Rendering-Bug einschleppen (Plan 015). Auf eine exakte Version pinnbar (z. B. `2.1.220`); ein Wechsel erzwingt eine Neuinstallation im Container                                                                 |

**KI-Zugang in der Sandbox (Plan 013/015).** Der zentrale Zugang wird pro
Nutzer verschlüsselt gespeichert (`claude-central`) und in **jede** Terminal-
Session direkt in die `docker exec`-Env gelegt (`terminalService.js`) — plus
per `tmux setenv -g` in die tmux-Session, damit auch `claude`/`codex` (die als
tmux-Kommando laufen und keine `.bashrc` sourcen) den Token sehen. Genau **eine**
Variable ist je Modus gesetzt:

- Abo/OAuth-Login → `CLAUDE_CODE_OAUTH_TOKEN` (der Access-Token; OAuth-Modus
  erneuert ihn lazy vor Ablauf über den Refresh-Token, `claudeOauthService`).
- API-Key → `ANTHROPIC_API_KEY`.

**Falle:** Ist ein Abo-/OAuth-Token aktiv, wird `ANTHROPIC_API_KEY` in der
exec-Env garantiert **leer** gesetzt — ein (etwa geerbter) API-Key würde sonst
den Abo-Token still schlagen und auf metered API-Billing umleiten. `--bare` wird
nie benutzt (es ignoriert `CLAUDE_CODE_OAUTH_TOKEN`).
...
| Variable                    | Default | Description                                                                                                                                                                                                                                                                                                                                                                     |
| --------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FLOW_LLM_TIMEOUT_MS         | 120000  | Wie lange eine Modell-Runde im CHAT-Agenten ZWISCHEN zwei Woertern stumm bleiben darf. Der Zaehler beginnt bei jedem Datenstueck neu. Seit 22.08.2026 in `compose.app.yaml` durchgereicht; davor bewirkte ein Wert in der `.env` nichts. Bis zum 22.08.2026 galt diese Zahl AUCH fuer Flows, dort aber mit anderer Bedeutung, siehe die naechste Zeile                          |
...
| PROJECT_GIT_DIR         | /arasul/projects | Wurzelverzeichnis der Projekt-Checkouts für die GitHub-Sync (Plan 013, B9) UND der **Projektablage** (`ablageService`); je Projekt ein Unterordner `<project_id>` (Host: `data/projects/<uuid>`, Compose-Mount). Explorer, Flows (`projekt://aktiv`) und Sandboxes (`/workspace/projekt`) arbeiten im selben Ordner; ein Git-gekoppeltes Projekt sieht hier schlicht sein Repo. Der Dienst legt es bei Bedarf an und klont fehlende Checkouts neu |
...
| ORDNER_SYNC_INTERVAL_MS | 20000            | Takt des Ein-Ordner-Modell-Abgleichs (`services/projects/ordnerSyncService.js`): Projektordner ↔ Wissensraum-Spiegel (Ordner→`knowledge_spaces`, indexierbare Dateien→`documents`/Pipeline). Zusätzlich stoßen Datei-Routen, Chat-Agent und Flow-Läufe den Abgleich sofort an                                                                                                                                                                     |
