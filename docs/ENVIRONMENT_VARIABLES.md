...
| SYSTEM_VERSION  | (aus dem Bau) | Die Fassung dieses Geräts. **Wird nicht von Hand gepflegt.** Seit Phase C10 (27.08.2026) setzt sie der Bau: `install.sh` aus `arasul-release.json` im Artefakt, `scripts/deploy/deploy-local.sh` aus Git (Tag auf HEAD, sonst `JJJJMMTT-<sha>`); beide schreiben sie in die `.env`. Form und Reihenfolge stehen in `scripts/lib/fassung.sh`. Ungesetzt liest ein Mensch überall `Vorserie` (`utils/version.js`), und das Gerät nimmt **keine** Aktualisierung über die Schnittstelle an (`validateManifest`: es kann nichts über die Verträglichkeit sagen). Siehe [ops/AUSLIEFERUNG.md](ops/AUSLIEFERUNG.md) |
| BUILD_HASH      | dev-build     | Git commit hash. **The default is a literal value, not a fallback** — it is shown verbatim as "Build" on the settings page. Gesetzt wird er zusammen mit `SYSTEM_VERSION` von `install.sh` und vom Deploy; von Hand: `BUILD_HASH=$(git rev-parse --short HEAD) docker compose up -d --build`                                                                                                                                                                                                                                                                                                                  |
...
| LLM_MODEL                   | `qwen3.8:27b-q4_K_M` | Standard der Kurzliste (seit J35 aus der Ollama-Bibliothek); unter 64 GB setzt `detect-platform.sh` `gemma4:e4b` (siehe `config/modelle/kurzliste.json`)                                |
...
### Remote HTTPS: one certificate for both networks

Remote access is a deliberate opt-in — the delivery default is LAN-only. Once
connected, the device is reachable at `https://<device>.<tailnet>.ts.net` and at
its raw Tailscale IP. **Traefik answers on both**, with the same certificate it
serves on the LAN: the one signed by the device CA created at first boot.

There used to be a `tailscale serve` path here (enabled automatically on
connect, plus three API endpoints). It is gone since 28.08.2026, Phase C10.
`tailscale serve --https=443` makes tailscaled bind `100.x.y.z:443`, and after
that Traefik cannot bind `0.0.0.0:443` any more — measured on the Orin: the
reverse proxy did not start and the device was unreachable **on its own LAN**.
A remote path that switches off the local one is not a remote path.

To get rid of the browser warning, distribute the device CA once — the same
step, and the only step, for LAN and remote: see
[NETZNAME_UND_ZERTIFIKAT.md](./ops/NETZNAME_UND_ZERTIFIKAT.md).
...
| FLOW_FREIGABE_FRIST_MINUTEN | 1440    | Phase C7: wie lange eine Freigabe-Anfrage aus einem Flow offen bleibt, wenn der Flow selbst keine `frist_minuten` nennt. 1440 Minuten sind ein Tag: lange genug, dass jemand einmal ins Buero kommt, kurz genug, dass ein vergessener Lauf nicht ewig steht. Danach endet der Lauf als `abgelaufen`. Das Warten kostet keine GPU (dieselbe Begruendung wie bei der Rueckfrage)         |
...
| Variable                         | Default                              | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MDNS_NAME                        | arasul                               | Der Netzname des Geräts, ohne `.local`. Er ist der DHCP-Name (NetworkManager meldet ihn dem Router, daher `https://arasul/`; der Systemname bleibt, J35), der Avahi-Name (`https://arasul.local/`), der Name im Gerätezertifikat und die Antwort von `GET /api/system/network`. Bei der Installation über `./install.sh --name <name>`, nachträglich über `sudo MDNS_NAME=<name> bash scripts/setup/setup-mdns.sh` plus `./arasul zertifikat`. Siehe [ops/NETZNAME_UND_ZERTIFIKAT.md](ops/NETZNAME_UND_ZERTIFIKAT.md) |
...
| MACHINE_ID_DATEI                 | /etc/machine-id                      | Host-Datei, die Compose nur lesbar nach `/arasul/host/machine-id` in dashboard-backend reicht (J32). Aus ihr bildet der Dienst den Fingerabdruck, an den eine Lizenz gebunden ist; ohne sie sieht er nur die MAC seines Containers, und die ist nach jedem Deploy eine andere.                                                                                                                                                                                                                                        |
| ENV_FILE_PATH                    | /arasul/config/.env                  | Path to runtime .env file                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| APPS_DIR                         | /arasul/apps                         | Wo die Apps am Gerät liegen, ein Ordner je Version (`<id>/<version>/app.json`, `<id>/<version>/frontend/`). Arasul liefert daraus `/apps/<id>/` aus. Read-only gemountet: geschrieben wird beim Deploy (Phase C5), gelesen bei jedem Seitenaufruf.                                                                                                                                                                                                                                                                    |
...
| SSH_PORT                         | 2222                                 | Der Port, auf den `scripts/security/haerten.sh` SSH legt (nur `NODE_ENV=production`, mit `sudo -n`). Ein Wechsel wird gemeldet: Warnung in der Ausgabe, `ARASUL_SSH_PORT=<port>`, `config/ssh-port`, Erstausgabe (J35).                                                                                                                                                                                                                                                                                               |
| ENABLE_SSH_HARDENING             | true                                 | `false` lässt die SSH-Härtung aus. Gelesen aus Umgebung oder `.env` (J35).                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ENABLE_FIREWALL                  | true                                 | `false` lässt die Firewall (ufw) aus. Gelesen aus Umgebung oder `.env` (J35).                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| MODELL_HOLEN                     | true                                 | `false`: der Bootstrap holt das Standardmodell (`LLM_MODEL`) nicht im Hintergrund. Von Hand: `./arasul modell` (J35).                                                                                                                                                                                                                                                                                                                                                                                                 |
| MODELL_HOLEN_STILLSTAND_SEKUNDEN | 300                                  | Nach so vielen Sekunden ohne Fortschritt gilt ein Download in `scripts/util/modell-holen.sh` als stehend: der Versuch wird beendet, `llm-service` neu gestartet und wiederholt (J35).                                                                                                                                                                                                                                                                                                                                 |
...
# System
SYSTEM_NAME=arasul
# Setzt der Bau, nicht die Hand: install.sh aus dem Artefakt, der Deploy aus
# Git. Ungesetzt heisst "Vorserie", und dann nimmt das Geraet keine
# Aktualisierung an.
