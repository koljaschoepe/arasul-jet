...
| N8N_SSRF_ALLOWED_HOSTNAMES        | llm-service,dashboard-backend,minio,embedding-service,document-indexer | Interne Hostnames, die trotz SSRF-Schutz erreichbar sind (Allowlist > Blocklist); postgres-db bewusst nicht                                                                                                                |
...
Network modes (`sandbox_projects.network_mode`, CHECK in migration 100): `isolated` (bridge, Internet only — GDPR-clean test environment), `internal` (backend network: LLM/DB), `infrastructure` (like `internal` plus platform repo rw + docker socket; **admin role only**, creation is audit-logged).
