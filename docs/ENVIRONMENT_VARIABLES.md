...
> Seit Phase B4 (26.08.2026) ist der Indexer ein reiner Extraktionsdienst:
> `GET /health` und `POST /extract-text` (multipart, Feld `file`). Kein
> Scan-Zyklus, keine Datenbank, kein MinIO, keine Chunks, kein Embedding,
> keine GPU. Die frueheren `DOCUMENT_INDEXER_*`-, `RAG_*`-, `SPACE_ROUTING_*`-
> und `GPU_VORRANG*`-Variablen haben keinen Leser mehr. `STAGE2_VRAM_FLOOR_MB`
> liest der Embedding-Dienst (`services/embedding-service/embedding_server.py`).
...
> Die Abschnitte Sandbox, Erweiterungs-Baukasten und KI-Zugang (`SANDBOX_*`,
> `EXTENSIONS_*`, `CLAUDE_*`) sind mit Phase B4 (26.08.2026) entfallen; die
> Dienste dazu gibt es nicht mehr.
