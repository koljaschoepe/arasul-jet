...
> **Ein Einstieg: `/plan`.** Du planst ein Feature, gibst nach dem Interview
> dein „go" — ab da läuft **alles** automatisch bis auf den Jetson. Kein
> `/ship`, kein `/deploy`, kein manueller Merge.
...
├─ 3 Plan-Datei  docs/plans/active/<slug>.md │
├─ 4 Approval  ← DEIN EINZIGER MANUELLER GATE ("go")
│
├─ 5 Branch  NNN-<slug>  +  autonome Ausführung
├─ 6 code-reviewer  (Critical-Findings = harter Stop)
├─ 7 Auto-Ship: Lint + Tests + Conventional Commit + Plan → done/
...
| Datei / Ort | Rolle |
| --- | --- |
| `.claude/commands/plan.md` | 8-Phasen-Command, treibt den ganzen Loop |
