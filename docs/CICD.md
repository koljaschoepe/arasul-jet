...
| **CI-Gate „CI Summary"**                                | Wartet über `needs` auf jeden Job von `test.yml` und ist nur grün, wenn alle grün sind (Docker darf fehlen, wenn kein Bau nötig ist). Nur bei grün merged GitHub.                                                                                                                                                                                                        |
...
| `.github/workflows/test.yml`                    | CI bei jedem PR und Push auf main. Der Job `changes` entscheidet, ob es nur Doku ist (dann überspringt jeder Job); `CI Summary` läuft immer, wartet auf alle Jobs und ist der einzige Check dieses Namens (seit 25.09.2026, `doku-summary.yml` ist entfernt) |
...
| GitHub-Ruleset „main geschuetzt“               | Seit 24.08.2026 aktiv: PR Pflicht, kein Force-Push, kein Löschen, Pflicht-Check „CI Summary“ — seit 25.09.2026 an die App GitHub Actions gebunden (`integration_id` 15368). Auto-Merge wartet damit auf den letzten Job |
