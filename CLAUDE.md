...
Internet (443) → Traefik → Dashboard-Frontend (React 19 SPA)
                         → Dashboard-Backend (Express API :3001)
                              ├─ PostgreSQL 16 (latest applied: 092, next: 093_*.sql)
...
| DB       | PostgreSQL 16 (next migration: `093_*.sql`)                  | `services/postgres/init/`                                     |
...
| Database    | `services/postgres/init/` (next migration: `093_*.sql`)   |
